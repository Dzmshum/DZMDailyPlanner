...
| 2.20 | **Черновик новой задачи** — ввод не теряется при клике вне модалки / повторном открытии *(v0.30.1)* | [ ] |
| 2.21 | **Черновик быстрого захвата** — текст восстанавливается после случайного закрытия *(v0.30.1)* | [ ] |
| 2.22 | После «Сохранить» черновик очищается; явная «Отмена» — по правилам confirm *(v0.30.1)* | [ ] |
...
| 4.2c1 | **Прогресс дня** — явный **%** + дробь `2/6`; compact-шапка читаема *(v0.30.2)* | [ ] |
...
| 9.9 | **`[`** — скрыть / показать сайдбар *(v0.31.1)* | [ ] |
| 9.10 | **`Ctrl+K`** — палитра команд (вкладки + действия) *(v0.31.1, фаза 2)* | [ ] |
| 9.11 | `1`–`8` работают при скрытом меню *(v0.31.1)* | [ ] |

---

## 9a. Скрываемое меню (v0.31.1)

| # | Проверка | Статус |
|---|----------|--------|
| 9a.1 | Режим **peek** — меню overlay, контент на всю ширину | [ ] |
| 9a.2 | Режим **rail** — узкая рейка иконок, счётчики видны | [ ] |
| 9a.3 | **Круговое меню** на логотипе — переход на вкладку | [ ] |
| 9a.4 | `sidebarMode` сохраняется в `plan.json` после перезапуска | [ ] |
| 9a.5 | Minimal-окно Electron не ломается | [ ] |
