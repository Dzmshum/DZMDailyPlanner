...
**Текущая версия:** v0.25.3 — см. [ROADMAP.md](./ROADMAP.md)
...
| `pnpm test` | Автотесты: календарь РФ + дейлики (40) + вложения (12) + месяц (9) + праздники (23) + проекты (5) + палитры (83) + экспорт TG (6) + настройки UI (20) |
...
| `2` | Повестка дня — задачи на дату, просроченные (свёрнуто), «Выполнено» |
...
| | |
|--|--|
| **Текущая версия** | **v0.25.3** — модалка настроек, кнопка ⚙, повестка/TG/иконки (v0.25.2) |
| **Тесты** | `pnpm test` — **198** проверок в **8** скриптах + календарь РФ 2025–2027 |
...
| Версия | Суть |
|--------|------|
| **v0.28** | Настройки UX 2.0 — «Моя тема» как часть палитр, таб «Поведение» |
| **v0.25.4** | Дни дейликов — выбор дней недели в настройках (логика уже есть) |
...
| `verify-daily-meetings.mjs` | 40 |
| `verify-attachments.mjs` | 12 |
| `verify-month-calendar.mjs` | 9 |
| `verify-holiday-labels.mjs` | 23 |
| `verify-projects.mjs` | 5 |
| `verify-palettes.mjs` | 83 |
| `verify-export-text.mjs` | 6 |
| `verify-settings-ui.mjs` | 20 |
